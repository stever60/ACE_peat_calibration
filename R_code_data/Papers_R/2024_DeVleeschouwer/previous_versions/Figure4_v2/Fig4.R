...
# Subsample
subsample_param <- c("Water_Content_pc", "Dry_mass", "Wet_density_g_cm3",
                     "Dry_density_g_cm3",	"DMAR_g_cm2_yr", "LOI550",	"C_org")
...
ACE_Subsample <- read_csv("Papers_R/2024_DeVleeschouwer/Figure4/Data/Input/ACE_Subsample_Composite.csv")
ACE_Subsample
...
# Standardise and centre dataframe - Z-scores
ACE_Subsample_Z <- ACE_Subsample
ACE_Subsample_Z[, subsample_param] <- scale(ACE_Subsample[, subsample_param], center = TRUE, scale = TRUE)
ACE_Subsample_Z
write.csv(ACE_Subsample_Z,"Papers_R/2024_DeVleeschouwer/Figure4/Data/Output/ACE_Subsample_comp_Z.csv", row.names = FALSE)

ACE_Subsample_long <- ACE_Subsample %>% 
  select(c(all_of(subsample_param), Site, depth, SH20_mean_age)) %>%
  pivot_longer(c(`subsample_param`), names_to = "param", values_to = "value") %>% 
  relocate(param, .before = depth)
ACE_Subsample_long

ACE_Subsample_long_Z <- ACE_Subsample_Z %>% 
  select(c(all_of(subsample_param), Site, depth, SH20_mean_age)) %>%
  pivot_longer(c(`subsample_param`), names_to = "param", values_to = "value") %>% 
  relocate(param, .before = depth)
ACE_Subsample_long_Z

library(tidypaleo) #https://cran.r-project.org/web/packages/tidypaleo/vignettes/strat_diagrams.html
theme_set(theme_paleo(12)) #theme_paleo

# Plot vs Depth - all subsample data
p1_ACE_Subsample_depth <- ggplot(ACE_Subsample_long, aes(x = value, y = depth)) +
  geom_lineh(colour = "black") +
  geom_point(shape = 21, fill = "white", color = "black", size = 1) +
  ylim(0, 410) +
  #scale_x_reverse() +
...
p5_C_Subsample <-
  ...
  theme(axis.text=element_text(size=12, colour = "black"), 
        ...
        title = element_text(size=12, colour = "black")) 
p5_C_Subsample
ggsave("Papers_R/2024_DeVleeschouwer/Figure4/Plots/Fig4/Fig4.5_Corg.pdf",
       height = c(24), width = c(8), dpi = 600, units = "cm")
...
p6_XRF_inc_coh <- 
  ...
  theme(axis.text=element_text(size=12, colour = "black"), 
        ...
        title = element_text(size=12, colour = "black")) 
p6_XRF_inc_coh
ggsave("Papers_R/2024_DeVleeschouwer/Figure4/Plots/Fig4/Fig4.6_inc_coh_XRF.pdf",
       height = c(24), width = c(8), dpi = 600, units = "cm")

ggarrange(p5_C_Subsample, p6_XRF_inc_coh,
          ncol = 4, nrow = 1, common.legend = TRUE)
ggsave("Papers_R/2024_DeVleeschouwer/Figure4/Plots/Fig4/Fig4_carbon.pdf",
