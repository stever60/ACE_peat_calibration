...
DATA NOTE:
  - Calibration responses (e.g., Ti_ICP) are natural-log transformed (ln ppm).
  - All model fitting and errors (RMSE, RMSEP) are computed in log-space.
  - Predictions on XRF_new are back-transformed with exp() to ppm.

PREDICTION COLUMNS IN *_preds.csv:
  - For each model, columns follow the pattern:
      Model_Element_Pred_ppm : predicted concentration in ppm
      Model_Element_L95_ppm  : lower 95% prediction bound in ppm
      Model_Element_U95_ppm  : upper 95% prediction bound in ppm
