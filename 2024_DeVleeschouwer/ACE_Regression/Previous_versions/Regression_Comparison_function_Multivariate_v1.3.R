...
run_full_regressions <- function(
    ...
    ICP_ppm,            # observed ICP in ppm, with e.g. Ti_ICP, Ti_ICP_sd, Site, SH20_age
    save_dir,           # base directory; function will create "Regression_Multivariate" inside
    var_suffix      = "_var",
    sd_suffix       = "_ICP_sd",
  ...
  for (el in elements) {
    ...
    readme_txt <- c(
      paste("Element:", el),
      "",
      "Calibration responses are ln(mg kg^-1); all RMSE/RMSEP in log-space.",
      "Predictions stored as *_Pred_ppm, *_L95_ppm, *_U95_ppm in mg kg^-1.",
      "",
  ...
  if (!is.null(XRF_new) &&
    ...
    for (el in elements) {
      ...
      # Global Y-range for log10 Y-equal plots
      ...
      # PLS-k columns for ribbons & exceedance tests
      ...
      for (s in site_levels_all) {
        ...
        # PLS-k per site (for ribbons & exceedances)
        if (has_pls_cols) {
          df_pls_site <- df_el %>%
            dplyr::filter(Site == s) %>%
            dplyr::select(
        ...
        # Exceedance check: PLS CI vs ICP ± SD range
        ast_df <- NULL
        if (!is.null(df_pls_site) &&
          ...
          exceed_idx <- with(
            df_pls_site,
            !is.na(L95_ppm) & !is.na(U95_ppm) &
              (L95_ppm < icp_lower_min | U95_ppm > icp_upper_max)
          )
          
          if (any(exceed_idx)) {
            ast_df <- df_pls_site[exceed_idx, , drop = FALSE]
          }
        }
        
        # y "top" for asterisks + n-label (free-scale plots)
        ...
        if (nrow(df_pred_site_all) == 0 && nrow(ace_site) == 0) {
          ...
          # PLS-k ribbon
          if (!is.null(df_pls_site) && nrow(df_pls_site) > 0) {
            p_all <- p_all +
              geom_ribbon(
                data = df_pls_site,
          ...
          # ICP error bars & path & points & median
          if (nrow(ace_site) > 0) {
            if (has_sd_el) {
              p_all <- p_all +
                geom_errorbar(
                  data = ace_site,
                  aes(x = SH20_age,
                      ymin = ICP_ppm - ICP_sd_ppm,
                      ymax = ICP_ppm + ICP_sd_ppm),
                  inherit.aes = FALSE,
                  colour = "pink",
                  width  = 0
                )
          ...
          # Asterisks where PLS CI exceeds ICP ± SD
          ...
          # n-label top-right
          if (!is.na(y_top_free) && !is.na(age_max)) {
            p_all <- p_all +
              annotate(
                "text",
                x = age_max,
        ...
        if (nrow(df_pred_site_best) == 0 && nrow(ace_site) == 0) {
          ...
          # ICP with error bars + median + points
          if (nrow(ace_site) > 0) {
            if (has_sd_el) {
              p_best <- p_best +
                geom_errorbar(
      ...
      # Helper: keep non-empty & shared x-axis via last panel
      ...
      combine_1col <- function(plot_list) {
        ...
        n <- length(keep)
        # Only bottom panel keeps x-axis label
        keep <- Map(
