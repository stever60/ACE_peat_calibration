...
# Section 21    (correlation diagnostics and plots to adi intepretation)
...
run_full_regressions <- function(
  ...
  for (el in elements) {
    ...
    for (mname in names(model_list)) {
      ...
      # -----------------------------------------------------------
      # Add ±SD error bars (Observed: XRF SD, Predicted: ICP-MS SD)
      # -----------------------------------------------------------
      
      # XRF SD column (e.g., Ti_sd)
      xrf_sd_col <- paste0(el, sd_suffix)         # e.g., "Ti_sd"
      
      # ICP-MS SD column (e.g., Ti_ICP_sd)
      ...
      # If XRF SD exists → horizontal bars
      if (xrf_sd_col %in% names(data)) {
        sd_vals <- data[[xrf_sd_col]]
        df_pred$Obs_low  <- df_pred$Observed - sd_vals
        df_pred$Obs_high <- df_pred$Observed + sd_vals
      }
      
      # If ICP SD exists → vertical bars
      if (icp_sd_col %in% names(data)) {
        sd_vals2 <- data[[icp_sd_col]]
        df_pred$Pred_low  <- df_pred$Predicted - sd_vals2
        df_pred$Pred_high <- df_pred$Predicted + sd_vals2
      }
      
      # Compute padded coordinates for annotation (top-left)
      x_left  <- min(df_pred$Observed,  na.rm = TRUE)
      y_top   <- max(df_pred$Predicted, na.rm = TRUE)
      
      # Small offsets (adjust if needed)
      x_offset <- 0.1 * (max(df_pred$Observed, na.rm = TRUE) -
                            min(df_pred$Observed, na.rm = TRUE))

      y_offset <- 0.2 * (max(df_pred$Predicted, na.rm = TRUE) -
                            min(df_pred$Predicted, na.rm = TRUE))
      
      # PLOT Pred vs Obs with Points on top of error bars
      p_po <- ggplot(df_pred, aes(Observed, Predicted, colour = Site)) +
        
        # Grey error bars drawn *behind* points
        geom_segment(aes(x = Obs_low,  xend = Obs_high,
                         y = Predicted, yend = Predicted),
                     inherit.aes = FALSE,
                     colour = "grey80", linewidth = 0.4) +
        
        geom_segment(aes(x = Observed, xend = Observed,
                         y = Pred_low, yend = Pred_high),
                     inherit.aes = FALSE,
                     colour = "grey80", linewidth = 0.4) +
        
        # Datapoints. of y = predicted x = observed 
        ...
        labs(
          x = expression("Obs. ICP-MS ± SD [Ln(mg kg"^{-1}*")]"),
          y = expression("Pred. ± 95% CI [Ln(mg kg"^{-1}*")]"),
    ...
    # ===============================================================
    # 8.7  SUMMARY PANELS (2×2) WITH SQUARE PANELS ON 2 A4 PAGES
    # ===============================================================
    ...
    get_plot_or_blank <- function(model_name) {
      if (!is.null(p_po_list[[model_name]])) {
        p_po_list[[model_name]] +
          coord_fixed()  # forces 1:1 aspect ratio
      } else {
    ...
    # -------------------------
    # SAVE BOTH PAGES (SQUARE PANELS) TO A4 PDF
    # -------------------------
    pdf(
      file   = file.path(element_dir, paste0(el, "_PredObs_Model_Summary_2page.pdf")),
      width  = 21 / 2.54,      # A4 width in inches
      height = 29.7 / 2.54     # A4 height in inches
    )
    ...
    # ===============================================================
    # 8.8  SUMMARY PANEL (2×4) OF ALL 8 MODELS ON ONE A4 PAGE
    # ===============================================================
    ...
    # Helper to fetch stored Pred vs Obs plot or blank if missing
    get_plot_or_blank <- function(model_name) {
      if (!is.null(p_po_list[[model_name]])) {
        p_po_list[[model_name]] + coord_fixed()  # enforce square aspect
      } else {
        ggplot() + theme_void() + coord_fixed()
      }
    }
    
    # Build list of plots in the desired order
    all_plots <- lapply(all_models_order, get_plot_or_blank)
    
    # Row 1: OLS, OLS_weighted
    row1 <- all_plots[[1]] | all_plots[[2]]
    # Row 2: WLS, WLS_weighted
    row2 <- all_plots[[3]] | all_plots[[4]]
    # Row 3: Bayes, RF
    row3 <- all_plots[[5]] | all_plots[[6]]
    # Row 24: PLS_kfold, PLS_LOO
    row4 <- all_plots[[7]] | all_plots[[8]]
    
    summary_panel_all <- (row1 / row2 / row3 / row4) +
      patchwork::plot_layout(guides = "collect") &
      theme(
        legend.position  = "bottom",
        legend.direction = "horizontal",
    ...
    # Save single-page A4 (landscape) panel into Calibration_outputs/<element>/
    ggsave(
      filename = paste0(el, "_PredObs_Model_Summary_1page_x.pdf"),
      path     = element_dir,
      plot     = summary_panel_all,
      width    = 21,    # A4 portrait width (cm)
      height   = 29.7,      # A4 portrait height (cm)
      units    = "cm"
    )
    
    # ===============================================================
    # 8.9  SUMMARY PANEL — PORTRAIT 2×4 (ALL 8 MODELS, TRUE SQUARE PANELS)
    # ===============================================================
    ...
    # Force TRUE square panel shape by applying theme(aspect.ratio = 1)
    make_square <- function(p) {
      p + theme(
        aspect.ratio = 1,        # <-- TRUE SQUARE PANEL
    ...
    # Save as A4 portrait
    ggsave(
      filename = paste0(el, "_PredObs_Model_Summary_1page.pdf"),
      path     = element_dir,
      plot     = summary_panel_all,
      width    = 21,      # A4 portrait width
      height   = 29.7,    # A4 portrait height
      units    = "cm"
    )
  ...
  # ===============================================================
  # 14. ACE DIAGNOSTIC EVALUATION vs ICP_ppm (NA-tolerant)
  # ===============================================================
  ...
  if (do_ACE_predictions && !is.null(ACE_dataset)) {
    ...
    for (el in elements) {
      ...
      for (mname in names(model_store[[el]])) {
        ...
        if (!any(ok)) {
          diag_out <- rbind(diag_out,
                            data.frame(Model = mname, R2 = NA, RMSE = NA))
          next
        }
        ...
        diag_out <- rbind(diag_out, data.frame(Model = mname, R2 = R2, RMSE = RMSE))
      }
      
      # =====================================================
      # NEW NA-TOLERANT BLOCK YOU REQUESTED
      # =====================================================
      diag_out <- diag_out %>%
        dplyr::filter(!(is.na(R2) & is.na(RMSE)))   # remove rows where BOTH are NA
      ...
      if (nrow(diag_out) == 0) {
        ...
        diag_out$Element <- el
        diag_out <- diag_out %>% arrange(desc(R2), RMSE) %>%
  ...
  # ===============================================================
  # 15. ACE PREDICTION DIAGNOSTIC PLOTS (WITH PROPER ERROR BARS)
  # 16. ACE SUMMARY PANELS
  # ===============================================================
  ...
  if (do_ACE_predictions && !is.null(ACE_dataset)) {
    ...
    for (el in elements) {
      ...
      for (mname in names(model_store[[el]])) {
        ...
        base_nm     <- paste0(mname, "_", el)
        pred_nm     <- paste0(base_nm, "_Pred_ppm")
        pred_low_nm <- paste0(base_nm, "_XRF_pred_lower")
        pred_up_nm  <- paste0(base_nm, "_XRF_pred_upper")
        
        if (!pred_nm %in% names(joined)) next
        
        df <- joined %>%
          dplyr::filter(!is.na(.data[[pred_nm]]), !is.na(.data[[obs_col]]))
        
        if (nrow(df) == 0) next
        
        df_pred <- df %>% dplyr::mutate(
          Observed  = .data[[obs_col]],
          Predicted = .data[[pred_nm]],
          Site      = Site,
          
          # ± 2 SD for ICP horizontal error bars (ppm)
          Obs_low   = Observed - ifelse(sd_col %in% names(df), .data[[sd_col]], 0),
          Obs_high  = Observed + ifelse(sd_col %in% names(df), .data[[sd_col]], 0),
          
          # Predicted model intervals (vertical)
          Pred_low  = .data[[pred_low_nm]],
          Pred_high = .data[[pred_up_nm]]
        )
        
        ok <- !is.na(df_pred$Observed) & !is.na(df_pred$Predicted)
        R2 <- RMSE <- NA_real_
        if (any(ok)) {
          obs  <- df_pred$Observed[ok]
          pred <- df_pred$Predicted[ok]
        ...
        stats_label <- paste0("R² = ", signif(R2,3),
                              "\nRMSE = ", signif(RMSE,4))
        
        x_left   <- min(df_pred$Observed,  na.rm = TRUE)
        x_offset <- 0.1 * diff(range(df_pred$Observed, na.rm = TRUE))
        y_top    <- max(df_pred$Predicted, na.rm = TRUE)
        y_offset <- 0.2 * diff(range(df_pred$Predicted, na.rm = TRUE))
        
        # ===========================
        # QUIET plotting: wrap in suppressMessages()
        # ===========================
        p1 <- suppressMessages({
          ggplot(df_pred, aes(Observed, Predicted, colour = Site)) +
            geom_segment(aes(x = Obs_low, xend = Obs_high,
                             y = Predicted, yend = Predicted),
                         colour = "grey80", linewidth = 0.4) +
            geom_segment(aes(x = Observed, xend = Observed,
            ...
            ggsci::scale_color_jco(name = "Site") +
            geom_abline(slope = 1, intercept = 0) +
            annotate("text", x = x_left - x_offset,
                     y = y_top + y_offset,
                     label = stats_label, hjust = 0, vjust = 1, size = 2.7) +
            labs(
              x = expression("Obs. ICP-MS ± SD (mg kg"^{-1}*")"),
              y = expression("Pred. ± 95% CI (mg kg"^{-1}*")"),
              title = paste(el, mname, "- ACE Predicted vs Observed")
            ) +
            theme_bw() + theme_small +
            coord_fixed()
        })
        
        ggsave(file.path(el_dir, paste0(el, "_", mname, "_ACE_PredVsObs.pdf")),
               p1, width = 12, height = 9, units = "cm")
        
        p_po_list_ace[[mname]] <- p1
        ...
        ggsave(file.path(el_dir, paste0(el, "_", mname, "_ACE_Residuals.pdf")),
      ...
      get_plot_ace <- function(name) {
        if (!is.null(p_po_list_ace[[name]]))
          p_po_list_ace[[name]]
        else
          ggplot() + theme_void() + coord_fixed()
      }
      ...
      # ----------------------- PAGE 1 -----------------------
      page1 <- suppressMessages({
        ((get_plot_ace("OLS") | get_plot_ace("OLS_weighted")) /
           (get_plot_ace("WLS") | get_plot_ace("WLS_weighted"))) +
          patchwork::plot_layout(guides = "collect") &
          theme(legend.position = "bottom") &
          guides(colour = guide_legend(nrow = 1))
      })
      ...
      # ----------------------- PAGE 2 -----------------------
      page2 <- suppressMessages({
        ((get_plot_ace("Bayes") | get_plot_ace("RF")) /
           (get_plot_ace("PLS_kfold") | get_plot_ace("PLS_LOO"))) +
          patchwork::plot_layout(guides = "collect") &
          theme(legend.position = "bottom") &
          guides(colour = guide_legend(nrow = 1))
      })
      ...
      suppressMessages({
        pdf(file.path(el_dir, paste0(el, "_ACE_PredObs_Model_Summary_2page.pdf")),
      ...
      model_order <- c("OLS","OLS_weighted","WLS","WLS_weighted",
                       "Bayes","RF","PLS_kfold","PLS_LOO")
      
      ace_plots <- lapply(model_order, get_plot_ace)
      
      summary_panel <- suppressMessages({
        ((ace_plots[[1]] | ace_plots[[2]]) /
           (ace_plots[[3]] | ace_plots[[4]]) /
           (ace_plots[[5]] | ace_plots[[6]]) /
           (ace_plots[[7]] | ace_plots[[8]])) +
          patchwork::plot_layout(guides = "collect") &
          theme(legend.position = "bottom") &
          guides(colour = guide_legend(nrow = 1))
      })
      ...
      ggsave(
        filename = paste0(el, "_ACE_PredObs_Model_Summary_1page_x.pdf"),
        path     = el_dir,
        plot     = summary_panel,
      ...
      make_square <- function(p) p + theme(aspect.ratio = 1)
      
      ace_plots_sq <- lapply(model_order, function(n) {
        if (!is.null(p_po_list_ace[[n]]))
          make_square(p_po_list_ace[[n]])
        else
          make_square(ggplot() + theme_void())
      })
      
      summary_panel_sq <- wrap_plots(
        ace_plots_sq, ncol = 2, nrow = 4, guides = "collect"
      ) &
        theme(legend.position="bottom") &
        guides(colour = guide_legend(nrow = 1))
      
      ggsave(
        filename = paste0(el, "_ACE_PredObs_Model_Summary_1page.pdf"),
        path     = el_dir,
        plot     = summary_panel_sq,
        width    = 21, height = 29.7, units = "cm"
      )
    }
  }
  ...
  # ===============================================================
  # 17. ACE GLOBAL SUMMARY (NA-tolerant, always writes CSVs)
  # ===============================================================
  ...
  if (do_ACE_predictions && !is.null(ACE_dataset)) {
    ...
    summary_pred   <- file.path(ace_root, "AllElements_ModelSummary_pred.csv")
    summary_legacy <- file.path(ace_root, "AllElements_ModelSummary.csv")
    
    best_pred      <- file.path(ace_root, "BestModels_PerElement_pred.csv")
    best_legacy    <- file.path(ace_root, "BestModels_PerElement.csv")
    
    ace_global <- data.frame()
    
    for (el in elements) {
      
      diag_file <- file.path(ace_root, el, paste0(el, "_ACE_diagnostics.csv"))
      if (!file.exists(diag_file)) next
      
      d <- read.csv(diag_file)
      
      # Remove rows where both metrics are NA
      d <- d %>% dplyr::filter(!(is.na(R2) & is.na(RMSE)))
      ...
      if (nrow(d) == 0) {
        ...
        d$Element <- el
        d <- d %>% arrange(desc(R2), RMSE) %>% mutate(Rank = row_number())
      }
      
      ace_global <- rbind(ace_global, d)
    }
    
    write.csv(ace_global, summary_pred,   row.names = FALSE)
    write.csv(ace_global, summary_legacy, row.names = FALSE)
    
    if (nrow(ace_global) > 0) {
      best_models <- ace_global %>% filter(Rank == 1)
    } else {
      best_models <- data.frame()
    }
    
    write.csv(best_models, best_pred,   row.names = FALSE)
    write.csv(best_models, best_legacy, row.names = FALSE)
  }
  
  # ===============================================================
  # 18. ACE ppm-scale summary (NA-tolerant, always writes CSVs)
  # ===============================================================
  ...
  if (do_ACE_predictions && !is.null(ACE_dataset)) {
    ...
    summary_pred   <- file.path(ace_root, "AllElements_ModelSummary_ppm_pred.csv")
    summary_legacy <- file.path(ace_root, "AllElements_ModelSummary_ppm.csv")
    
    # Remove rows where Element missing (placeholders allowed)
    ace_valid <- ace_global %>% dplyr::filter(!is.na(Element))
    
    if (nrow(ace_valid) == 0) {
      write.csv(data.frame(), summary_pred,   row.names = FALSE)
      write.csv(data.frame(), summary_legacy, row.names = FALSE)
      message("No valid ACE diagnostics — empty ppm summary written.")
      
    } else {
      
      elem_means <- lapply(elements, function(el) {
        obs_col <- paste0(el, "_ICP")
        if (!obs_col %in% names(ICP_ppm))
          return(data.frame(Element = el, Mean_ppm = NA_real_))
        
        data.frame(Element = el, Mean_ppm = mean(ICP_ppm[[obs_col]], na.rm = TRUE))
      }) %>% dplyr::bind_rows()
      
      ace_ppm <- ace_valid %>%
        left_join(elem_means, by = "Element") %>%
        mutate(
          RMSE_abs_ppm = RMSE,
          RMSE_factor  = RMSE / Mean_ppm
        )
      
      write.csv(ace_ppm, summary_pred,   row.names = FALSE)
      write.csv(ace_ppm, summary_legacy, row.names = FALSE)
      
      message("=== ACE ppm summary complete ===")
    }
  }
  # ===============================================================
  # 19. MULTI-ELEMENT CORRELATION DIAGNOSTICS
  # ===============================================================
  ...
  # -------------------------------------------------------------------
  # 19.1 CORRELATION MATRIX OF RAW ICP LOG VALUES
  # -------------------------------------------------------------------
  ...
  # -------------------------------------------------------------------
  # 19.2 CORRELATION BETWEEN MODEL PREDICTIONS (CALIBRATION DATA)
  # -------------------------------------------------------------------
  ...
  # ===============================================================
  # 20. WRITE GLOBAL README FILE (placed in Regression_Multivariate)
  # ===============================================================
