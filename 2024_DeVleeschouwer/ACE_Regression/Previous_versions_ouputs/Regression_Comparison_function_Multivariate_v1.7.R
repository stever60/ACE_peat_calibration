...
# Section 15    ACE PREDICTION DIAGNOSTIC PLOTS (FULLY PATCHED, OPTION S3)
...
# Section 19    ACE DIAGNOSTICS DASHBOARD  (New)
...
run_full_regressions <- function(
  ...
  for (el in elements) {
    ...
    for (mname in names(model_list)) {
      ...
      # -----------------------------------------------------------
      # Add ±SD error bars (Observed: ICP-MS SD, Predicted: XRF SD)
      # -----------------------------------------------------------
      ...
      # XRF SD → vertical bars (Predicted / y-axis)
      if (xrf_sd_col %in% names(data)) {
        sd_xrf <- data[[xrf_sd_col]]
        df_pred$Pred_low  <- df_pred$Predicted - sd_xrf
        df_pred$Pred_high <- df_pred$Predicted + sd_xrf
      }
    ...
    get_plot_or_blank <- function(model_name) {
      if (!is.null(p_po_list[[model_name]])) {
        p_po_list[[model_name]] + coord_fixed()
      } else {
        ggplot() + theme_void() + coord_fixed()
      }
    }
    ...
    # ===============================================================
    # 8.9  SUMMARY PANEL — PORTRAIT 2×4 (TRUE SQUARE PANELS)
    # ===============================================================
  ...
  # ===============================================================
  # 14. ACE DIAGNOSTIC EVALUATION vs ICP_ppm  (NA-safe)
  # ===============================================================
  ...
  if (do_ACE_predictions && !is.null(ACE_dataset)) {
    ...
    for (el in elements) {
      ...
      if (!sd_col %in% names(joined)) joined[[sd_col]] <- NA_real_
      ...
      # --- Remove all-NA rows but keep element placeholder ---
      diag_out <- diag_out %>% dplyr::filter(!(is.na(R2) & is.na(RMSE)))
      
      if (nrow(diag_out) == 0) {
        diag_out <- data.frame(Model=NA, R2=NA, RMSE=NA)
      }
  ...
  # ===============================================================
  # 15. ACE PREDICTION DIAGNOSTIC PLOTS (FULLY PATCHED, OPTION S3)
  # ===============================================================
  
  if (do_ACE_predictions && !is.null(ACE_dataset)) {
    
    message("=== Creating ACE diagnostic plots (patched Section 15) ===")
    
    ace_root <- file.path(all_base, "Calibration_pred")
    
    # Ensure storage lists exist for summary page generation
    if (!exists("p_po_list_ace_orig")) p_po_list_ace_orig <- list()
    if (!exists("p_po_list_ace_pc"))   p_po_list_ace_pc   <- list()
    
    # Scientific formatting
    sci_fmt <- function(x) format(x, scientific = TRUE, digits = 3)
    ...
    for (el in elements) {
      ...
      # Merge ACE predictions with observed ICP
      joined <- ace_df %>%
        dplyr::left_join(
          ICP_ppm %>% dplyr::select(Site, depth, SH20_age, !!sym(obs_col), !!sym(sd_col)),
          by = c("Site", "depth", "SH20_age")
      ...
      # Axis labels (no x10^3 text)
      axis_x_lab <- expression("Observed ICP-MS (mg kg"^{-1}*")")
      axis_y_lab <- expression("Predicted (mg kg"^{-1}*")")
      
      # Prepare model-level storage lists
      p_po_list_ace_orig[[el]] <- list()
      p_po_list_ace_pc[[el]]   <- list()
      ...
      for (mname in names(model_store[[el]])) {
        ...
        L95_nm      <- paste0(base_nm, "_L95_ppm")
        U95_nm      <- paste0(base_nm, "_U95_ppm")
        L95pc_nm    <- paste0(base_nm, "_L95_pc_ppm")
        U95pc_nm    <- paste0(base_nm, "_U95_pc_ppm")
        
        needed <- c(pred_nm, L95_nm, U95_nm, L95pc_nm, U95pc_nm)
        if (!all(needed %in% names(joined))) next
        ...
        # --- Compute stats ---
        pred_vals <- df[[pred_nm]]
        obs_vals  <- df[[obs_col]]
        ok <- !is.na(pred_vals) & !is.na(obs_vals)
        
        RMSE_val  <- sqrt(mean((obs_vals[ok] - pred_vals[ok])^2))
        RMSEP_val <- RMSE_val  # No cross-validation in ACE
        
        R2_val <- 1 - sum((obs_vals - pred_vals)^2) /
          sum((obs_vals - mean(obs_vals))^2)
        
        stats_lab <- paste0(
          "R² = ", signif(R2_val, 3),
          "\nRMSEP = ", signif(RMSEP_val, 4),
          "\nRMSE = ", signif(RMSE_val, 4)
        )
        ...
        df_plot <- df %>%
          dplyr::mutate(
            ...
            Vlow_orig  = .data[[L95_nm]],
            Vhigh_orig = .data[[U95_nm]],
            
            Vlow_pc    = .data[[L95pc_nm]],
            Vhigh_pc   = .data[[U95pc_nm]],
            
            Hlow  = Obs_s - ifelse(is.na(.data[[sd_col]]), 0, .data[[sd_col]]),
            Hhigh = Obs_s + ifelse(is.na(.data[[sd_col]]), 0, .data[[sd_col]])
          )
        
        # --- Base plot builder ---
        make_base_plot <- function(df2) {
          ggplot(df2, aes(Obs_s, Pred_s, colour = Site)) +
            geom_abline(slope = 1, intercept = 0) +
            ggsci::scale_color_jco(name = "Site") +
            theme_bw() + theme_small +
            labs(
            ...
            scale_x_continuous(labels = sci_fmt) +
            scale_y_continuous(labels = sci_fmt) +
            annotate(
              "text",
              x = min(df2$Obs_s, na.rm = TRUE),
              y = max(df2$Pred_s, na.rm = TRUE),
              label = stats_lab,
              hjust = 0, vjust = 1,
              size = 2.7
            ) +
        ...
        # --- Error bar adders ---
        add_vert_orig <- function(p, df2)
          p + geom_segment(aes(x = Obs_s, xend = Obs_s,
                               y = Vlow_orig, yend = Vhigh_orig),
                           inherit.aes = FALSE, colour = "grey70", linewidth = 0.4)
        
        add_vert_pc <- function(p, df2)
          p + geom_segment(aes(x = Obs_s, xend = Obs_s,
                               y = Vlow_pc, yend = Vhigh_pc),
                           inherit.aes = FALSE, colour = "grey70", linewidth = 0.4)
        
        add_horiz <- function(p, df2)
          p + geom_segment(aes(x = Hlow, xend = Hhigh,
                               y = Pred_s, yend = Pred_s),
                           inherit.aes = FALSE, colour = "grey70", linewidth = 0.4)
        
        # ============================================================
        #  PLOT 1 — ORIGINAL PI (NO ERROR BARS)
        # ============================================================
        p1 <- make_base_plot(df_plot) + geom_point(size = 1)
        f1 <- file.path(el_dir, paste0(el, "_", mname, "_ACE_PredVsObs.pdf"))
        ggsave(f1, p1, width = 12, height = 12, units = "cm")
        
        # ============================================================
        #  PLOT 2 — ORIGINAL PI WITH ERROR BARS (_err)
        # ============================================================
        p2 <- make_base_plot(df_plot) %>% 
          add_vert_orig(df_plot) %>% 
          add_horiz(df_plot) +
          geom_point(size = 1)
        f2 <- file.path(el_dir, paste0(el, "_", mname, "_ACE_PredVsObs_err.pdf"))
        ggsave(f2, p2, width = 12, height = 12, units = "cm")
        
        # store for Section 16 summaries
        p_po_list_ace_orig[[el]][[mname]] <- p2
        
        # ============================================================
        #  PLOT 3 — PERCENTAGE PI (NO ERROR BARS)
        # ============================================================
        p3 <- make_base_plot(df_plot) %>%
          add_vert_pc(df_plot) +
          geom_point(size = 1)
        f3 <- file.path(el_dir, paste0(el, "_", mname, "_ACE_PredVsObs_pc.pdf"))
        ggsave(f3, p3, width = 12, height = 12, units = "cm")
        
        # ============================================================
        #  PLOT 4 — PERCENTAGE PI WITH ERROR BARS (_pc_err)
        # ============================================================
        p4 <- make_base_plot(df_plot) %>%
          add_vert_pc(df_plot) %>%
          add_horiz(df_plot) +
          geom_point(size = 1)
        f4 <- file.path(el_dir, paste0(el, "_", mname, "_ACE_PredVsObs_pc_err.pdf"))
        ggsave(f4, p4, width = 12, height = 12, units = "cm")
        
        # store for summary page
        p_po_list_ace_pc[[el]][[mname]] <- p4
  ...
  # ===============================================================
  # 16. ACE SUMMARY PANELS (ORIGINAL + PERCENTAGE PI, SQUARE PANELS)
  # ===============================================================
  
  if (do_ACE_predictions && !is.null(ACE_dataset)) {
    
    message("=== Creating ACE summary panels (patched Section 16) ===")
    
    ace_root <- file.path(all_base, "Calibration_pred")
    
    model_order <- c(
      "OLS", "OLS_weighted", "WLS", "WLS_weighted",
    ...
    get_plot <- function(store, el, m) {
      if (!is.null(store[[el]][[m]])) make_square(store[[el]][[m]])
      else make_square(ggplot() + theme_void())
    }
    ...
    for (el in elements) {
      ...
      # Skip elements without ACE plots
      if (!is.list(p_po_list_ace_orig[[el]])) next
      if (!is.list(p_po_list_ace_pc[[el]]))   next
      
      # -------------------------
      # ORIGINAL PI SUMMARY
      # -------------------------
      orig_plots <- lapply(model_order, function(m)
        get_plot(p_po_list_ace_orig, el, m))
      
      orig_summary <- wrap_plots(
        orig_plots, ncol = 2, nrow = 4, guides = "collect"
      ) &
        theme(legend.position = "bottom") &
        guides(colour = guide_legend(nrow = 1))
      
      ggsave(
        filename = file.path(el_dir, paste0(el, "_ACE_Model_Summary_original.pdf")),
        plot     = orig_summary,
        width    = 21,
        height   = 29.7,
        units    = "cm"
      ...
      # -------------------------
      # PERCENTAGE PI SUMMARY
      # -------------------------
      pc_plots <- lapply(model_order, function(m)
        get_plot(p_po_list_ace_pc, el, m))
      
      pc_summary <- wrap_plots(
        pc_plots, ncol = 2, nrow = 4, guides = "collect"
      ) &
        theme(legend.position = "bottom") &
        guides(colour = guide_legend(nrow = 1))
      
      ggsave(
        filename = file.path(el_dir, paste0(el, "_ACE_Model_Summary_percentage.pdf")),
        plot     = pc_summary,
        width    = 21,
        height   = 29.7,
        units    = "cm"
  ...
  if (do_ACE_predictions && !is.null(ACE_dataset)) {
    ...
    for (el in elements) {
      ...
      if (nrow(d) == 0) {
        d <- data.frame(Model=NA, R2=NA, RMSE=NA)
      }
      
      d$Element <- el
      
      # Only rank rows with non-NA R2
      d <- d %>% arrange(desc(R2), RMSE)
  ...
  # ===============================================================
  # 18. ACE ppm-scale summary (NA-safe)
  # ===============================================================
  ...
  if (do_ACE_predictions && !is.null(ACE_dataset)) {
    ...
    if (!exists("ace_global") || nrow(ace_global) == 0) {
      message("No ACE summary available.")
      next
    }
    
    ace_valid <- ace_global %>% dplyr::filter(!is.na(Element))
    if (nrow(ace_valid) == 0) next
    ...
    elem_means_list <- lapply(elements, function(el) {
      ...
      vals <- vals[!is.na(vals)]
      if (length(vals)==0) return(data.frame(Element=el, Mean_ppm=NA_real_))
  ...
  # ===============================================================
  # 19. ACE DIAGNOSTICS DASHBOARD  (New)
  # ===============================================================
