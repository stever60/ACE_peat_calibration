...
run_full_regressions <- function(
  ...
  if (!is.null(XRF_new) &&
    ...
    multivar_labels <- c("Bayes","RF","PLS(LOO)","PLS(k)")
    layout_sites    <- dynamic_layout(length(site_levels_all))
    ...
    for (el in elements) {
      ...
      y_col <- paste0(el, "_ICP")
      ace_all <- data %>%
        dplyr::filter(!is.na(SH20_mean_age),
                      !is.na(.data[[y_col]])) %>%
        dplyr::mutate(ICP_ppm = exp(.data[[y_col]]))
      
      if (nrow(df_long_all)==0) next
      ...
      for (s in site_levels_all) {
        ...
        if (nrow(df_pred_site_all)==0 && nrow(ace_site)==0) {
          ...
          if (nrow(ace_site)>0) {
            p_all <- p_all +
              geom_path(data=ace_site,
                        aes(x=SH20_mean_age, y=ICP_ppm,
                            colour="ICP-MS (Observed)"),
                        linewidth=0.6, inherit.aes=FALSE) +
              geom_point(data=ace_site,
                         aes(x=SH20_mean_age, y=ICP_ppm,
                             colour="ICP-MS (Observed)"),
                         shape=21, fill="white", stroke=0.4, size=1.4,
                         inherit.aes=FALSE)
      ...
      # Combine ALL models page
      used <- sum(!vapply(plot_list_all,
                          function(p) inherits(p$theme, "theme_void"),
                          logical(1)))
      ...
      if (used>0) {
        combined_all <- wrap_plots(
          plot_list_all,
          ncol = layout_sites$ncol,
          nrow = layout_sites$nrow,
          guides = "collect"
        ) & theme(legend.position = "bottom")
      ...
      # Combine BEST models page
      used <- sum(!vapply(plot_list_best,
                          function(p) inherits(p$theme, "theme_void"),
                          logical(1)))
      
      if (used>0) {
        combined_best <- wrap_plots(
          plot_list_best,
          ncol = layout_sites$ncol,
          nrow = layout_sites$nrow,
          guides = "collect"
