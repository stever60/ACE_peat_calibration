...
# v. 1.2.3
# - changed pred vs obs back to jco site colours
...
# -------------------------------------------------------------------------
# Import datasets
# • ACE matched XRF-CS log (element/inc) file & checks names/data  ----------------------------
...
# • ICP_obs for plotting measured ICP-MS and error data in  ---------------
...
run_full_regressions <- function(
    elements,           # e.g., c("Ca", "Ti","Fe","Mn","Sr","Zr")
    data,               # ACE_dataset
    XRF_pred = NULL,     # New XRF dataset
    save_dir,           # User-defined output folder
    var_suffix    = "_var",
    sd_suffix     = "_ICP_sd",
  ...
  check_dataset_integrity <- function(data, el, y_col, x_col, var_col, sd_col) {
    ...
    for (v in required)
      if (!v %in% names(data)) errs <- c(errs, paste("Missing:", v))
  ...
  # ===============================================================
  # 6. CROSS-VALIDATION (10-fold RMSE)
  # ===============================================================
  ...
  for (el in elements) {
    ...
    # --------------------------------------------------------------
    # 10.1 FIT ALL MODELS (no skipping)
    # --------------------------------------------------------------
    ...
    # --------------------------------------------------------------
    # 10.3 INITIALISE PREDICTION DF
    # --------------------------------------------------------------
    
    do_predictions <- !is.null(XRF_pred)
    preds_df <- if (do_predictions) XRF_pred else NULL
    ...
    for (mname in names(model_list)) {
      ...
      # Predictions on calibration data (safe)
      pred <- tryCatch({
        if (inherits(m, "mvr"))
      ...
      if (do_predictions) {
        ...
        pred_new_log <- tryCatch({
          if (inherits(m, "mvr"))
            as.numeric(predict(m, ncomp = attr(m, "best_ncomp"), newdata = XRF_pred))
          else if (!is.null(m))
            as.numeric(predict(m, newdata = XRF_pred))
          else
            rep(NA_real_, nrow(XRF_pred))
        }, error = function(e) rep(NA_real_, nrow(XRF_pred)))
        
        lower_log <- pred_new_log - 1.96 * RMSE
        upper_log <- pred_new_log + 1.96 * RMSE
        
        # Proper intervals for lm
        if (mname %in% c("OLS","WLS","OLS_weighted","WLS_weighted") && !is.null(m)) {
          pi_obj <- tryCatch(
            predict(m, newdata = XRF_pred, interval = "prediction", level = 0.95),
            error=function(e) NULL
          )
          if (!is.null(pi_obj)) {
    ...
    readme_txt <- c(
      paste("Element:", el),
      "",
      "All calibration responses are log-transformed.",
      "RMSE & RMSEP computed in log-space.",
      "Predictions stored in *_Pred_ppm, *_L95_ppm, *_U95_ppm.",
      "",
      "Model types:",
      "  OLS, WLS, OLS(wt), WLS(wt), Bayes, RF, PLS(LOO), PLS(k).",
      "",
      "Ranking:",
      "  Highest R² → lowest RMSEP → lowest RMSE."
    )
  ...
  if (!is.null(XRF_pred) && length(preds_store) > 0) {
    
    has_site     <- "Site" %in% names(XRF_pred)
    has_depth    <- "depth" %in% names(XRF_pred)
    has_age_new  <- "SH20_age" %in% names(XRF_pred)
    has_age_cal  <- "SH20_age" %in% names(data)
    ...
    if (has_site) {
      site_levels <- sort(unique(XRF_pred$Site))
      ...
      for (s in site_levels) {
        ...
        if (has_depth) {
          ...
          for (el in elements) {
            ...
            p_best <- ggplot(df_long_best, aes(Pred_ppm, depth,
                                               colour=Model, group=Model)) +
              geom_path(linewidth=0.4) +
              scale_y_reverse() +
              labs(x=bquote(.(el)~"(mg"~kg^{-1}*")"),
            ...
            depth_plots_all[[el]]  <- p_all
            depth_plots_best[[el]] <- p_best
          }
          
          # Combine layouts dynamically
          if (length(depth_plots_all) > 0) {
            panels <- lapply(elements, function(e)
              depth_plots_all[[e]] %||% (ggplot() + theme_void()))
            
            used <- sum(vapply(panels,
                               function(p) !inherits(p$theme, "theme_void"),
                               logical(1)))
            
            if (used>0) {
              lay <- dynamic_layout(used)
              combined <- wrap_plots(panels,
                                     ncol = lay$ncol,
                                     nrow = lay$nrow,
                                     guides = "collect") &
          ...
          if (length(depth_plots_best) > 0) {
            panels <- lapply(elements, function(e)
              depth_plots_best[[e]] %||% (ggplot()+theme_void()))
        ...
        if (has_age_new && has_age_cal) {
          ...
          for (el in elements) {
            ...
            p_all <- ggplot(df_long, aes(Pred_ppm, SH20_age,
                                         colour=Model, group=Model)) +
              geom_path(linewidth=0.4) +
              labs(x=bquote(.(el)~"(mg"~kg^{-1}*")"),
                   y="Age (cal a BP)",
            ...
            if (nrow(ace_overlay_age)>0) {
              p_all <- p_all +
                geom_path(data=ace_overlay_age,
                          aes(x=ICP_ppm, y=SH20_age,
                              colour="ICP-MS (Observed)"),
                          linewidth=0.6,
                          inherit.aes=FALSE) +
                geom_point(data=ace_overlay_age,
                           aes(x=ICP_ppm, y=SH20_age,
                               colour="ICP-MS (Observed)"),
                           shape=21, fill="white", stroke=0.4, size=1.4,
                           inherit.aes=FALSE)
            ...
            p_best <- ggplot(df_long_best, aes(Pred_ppm, SH20_age,
                                               colour=Model, group=Model)) +
              geom_path(linewidth=0.4) +
              labs(x=bquote(.(el)~"(mg"~kg^{-1}*")"),
                   y="Age (cal a BP)",
            ...
            age_plots_all[[el]]  <- p_all
            age_plots_best[[el]] <- p_best
          }
          
          # Combine AGE (all models)
          if (length(age_plots_all)>0) {
            panels <- lapply(elements, function(e)
              age_plots_all[[e]] %||% (ggplot()+theme_void()))
          ...
          # Combine AGE (best multivariate)
          if (length(age_plots_best)>0) {
            panels <- lapply(elements, function(e)
              age_plots_best[[e]] %||% (ggplot()+theme_void()))
  ...
  if (!is.null(XRF_pred) &&
      ...
      "Site" %in% names(XRF_pred) &&
      "SH20_age" %in% names(XRF_pred) &&
      "SH20_age" %in% names(data)) {
    ...
    # Global AGE range across calibration + prediction datasets
    age_all <- c(XRF_pred$SH20_age, data$SH20_age)
    ...
    site_levels_all <- sort(unique(c(as.character(XRF_pred$Site),
    ...
    for (el in elements) {
      ...
      ace_all <- data %>%
        dplyr::filter(!is.na(SH20_age),
                      !is.na(.data[[y_col]])) %>%
        dplyr::mutate(
          ICP_ppm = exp(.data[[y_col]]),
          ICP_site_median = median(exp(.data[[y_col]]), na.rm = TRUE)
        )
      
      # ============================================================
      # GLOBAL Y-RANGE FOR THIS ELEMENT (for Y-equal log10 plots)
      # ============================================================
      ...
      y_min_el <- if (length(y_all_vals)) min(y_all_vals) else NA_real_
      y_max_el <- if (length(y_all_vals)) max(y_all_vals) else NA_real_
      
      # Log ticks at powers of ten, with scientific-notation labels
      ...
      # =====================================================================
      # CONSTRUCT ONE-COLUMN PANELS PER SITE
      # =====================================================================
      ...
      for (s in site_levels_all) {
        ...
        site_median <- if (nrow(ace_site)) median(ace_site$ICP_ppm, na.rm = TRUE) else NA_real_
        ...
        if (nrow(df_pred_site_all)==0 && nrow(ace_site)==0) {
          ...
          if (!is.na(age_min) && !is.na(age_max))
            p_all <- p_all + coord_cartesian(xlim=c(age_min, age_max))
          
          # ICP-MS lines and points
          if (nrow(ace_site)>0) {
            p_all <- p_all +
              geom_path(
                data = ace_site,
              ...
              # horizontal reference line at median ICP concentration
              geom_hline(
                yintercept = site_median,
                linetype = "dashed",
                colour = "grey40",
                linewidth = 0.4
              )
        ...
        if (nrow(df_pred_site_best)==0 && nrow(ace_site)==0) {
          ...
          if (!is.na(age_min) && !is.na(age_max))
            p_best <- p_best + coord_cartesian(xlim=c(age_min, age_max))
      ...
      # Save: MULTIVARIATE ONLY (Y-equal, log10)
  ...
  # ===============================================================
  # 13. GLOBAL SUMMARY ACROSS ALL ELEMENTS
  # ===============================================================
  
  if (nrow(global_summary) > 0) {
    
    global_ranked <- global_summary %>%
      dplyr::arrange(Element, dplyr::desc(R2), RMSEP, RMSE) %>%
  ...
  if (exists("global_ranked") && nrow(global_ranked) > 0) {
    ...
    elem_means <- lapply(elements, function(el) {
      ...
      if (!y_col %in% names(data))
        return(data.frame(Element=el, Mean_log_ppm=NA, Mean_ppm=NA))
    ...
    elem_means_df <- do.call(rbind, elem_means)
  ...
  message("Results saved to: ", save_dir)
...
run_full_regressions(
  ...
  data     = ACE_dataset,
  XRF_pred  = XRF_pred,
  save_dir = "/Users/sjro/Desktop/Regression_multivariate"
)
