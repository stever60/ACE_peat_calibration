...
# -------------------------------------------------------------------------
# Calibration model comparison function  --------------------
...
run_full_regressions <- function(
  ...
  for (el in elements) {
    ...
    for (mname in names(model_list)) {
      ...
      # ------------------------------------------------------------------
      # NEW: Compute RMSE in PPM scale (for comparability with ACE)
      # ------------------------------------------------------------------
      ...
      # ------------------------------------------------------------------
      # NEW: RMSEP in ppm scale (convert CV log predictions into ppm)
      # ------------------------------------------------------------------
      ...
      # -----------------------------------------------------------
      # NEW: Pred vs Obs plots WITHOUT error bars (per model)
      # -----------------------------------------------------------
      ...
      # Save new version
      ggsave(
        file.path(element_dir, paste0(el, "_", mname, "_PredVsObs_noerr.pdf")),
        p_noerr,
    ...
    get_plot_noerr <- function(model_name) {
      if (!is.null(p_po_list_noerr[[model_name]])) {
        p_po_list_noerr[[model_name]] + coord_fixed()
      } else {
        ggplot() + theme_void() + coord_fixed()
      }
    }
  ...
  if (do_ACE_predictions && !is.null(ACE_dataset)) {
    ...
    for (el in elements) {
      ...
      # Must exist in ICP_ppm
      if (!(obs_col %in% names(ICP_ppm))) next
      
      # ---------------------------------------------------------------
      # SAFE JOIN (explicit dplyr::select prevents masking errors)
      # ---------------------------------------------------------------
      joined <- ace_df %>%
        dplyr::left_join(
      ...
      # ---------------------------------------------------------------
      # Loop through all model predictions for this element
      # ---------------------------------------------------------------
      for (mname in names(model_store[[el]])) {
        ...
        # rows with valid obs + pred
        df <- joined %>% dplyr::filter(!is.na(.data[[obs_col]]),
                                       !is.na(.data[[pred_nm]]))
        if (nrow(df) == 0) next
        
        # -----------------------------------------------------------
        # Compute ACE ppm-scale metrics
        # -----------------------------------------------------------
        obs  <- df[[obs_col]]
        pred <- df[[pred_nm]]
        ...
        RMSEP_ppm <- NA_real_  # (ACE RMSEP not used)
        
        stats_label <- paste0(
          "R² = ", ifelse(is.na(R2_ppm), "NA", signif(R2_ppm, 3)),
          "\nRMSE = ", signif(RMSE_ppm, 4), " ppm"
        )
        ...
        # -----------------------------------------------------------
        # Equal axis limits + scientific labels
        # -----------------------------------------------------------
        axis_min <- min(c(df[[obs_col]], df[[pred_nm]]), na.rm = TRUE)
        axis_max <- max(c(df[[obs_col]], df[[pred_nm]]), na.rm = TRUE)
        ...
        # Ensure nested storage exists
        if (!el %in% names(p_po_list_ace)) p_po_list_ace[[el]] <- list()
        if (!mname %in% names(p_po_list_ace[[el]])) p_po_list_ace[[el]][[mname]] <- list()
        
        # ===========================================================
        # 1. NO ERROR PLOT
        # ===========================================================
        p_no_err <- ggplot(df, aes(x = .data[[obs_col]], y = .data[[pred_nm]], colour = Site)) +
          geom_point(size = 1.4) +
          sci_axes +
        ...
        ggsave(file.path(el_dir, paste0(el, "_", mname, "_ACE_PredVsObs_no_err.pdf")),
        ...
        if (all(c(sd_col, Lnm, Unm) %in% names(df))) {
          ...
          p_abs_err <- ggplot(df, aes(x = .data[[obs_col]], y = .data[[pred_nm]], colour = Site)) +
            ...
            # Vertical predicted error (absolute ppm)
            geom_segment(aes(
              x = .data[[obs_col]],
              xend = .data[[obs_col]],
              y = .data[[Lnm]],
          ...
          ggsave(file.path(el_dir, paste0(el, "_", mname, "_ACE_PredVsObs_abs_err.pdf")),
        ...
        # ===========================================================
        # 3. PERCENT ERROR PLOT
        # ===========================================================
        if (all(c(Lpc_nm, Upc_nm) %in% names(df))) {
          
          p_pc_err <- ggplot(df, aes(x = .data[[obs_col]], y = .data[[pred_nm]], colour = Site)) +
          ...
          ggsave(file.path(el_dir, paste0(el, "_", mname, "_ACE_PredVsObs_pc_err.pdf")),
  ...
  if (do_ACE_predictions && !is.null(ACE_dataset)) {
    ...
    if (!exists("p_po_list_ace")) {
      ...
      for (el in elements) {
        ...
        # function to enforce square and valid plotting
        sq <- function(p) {
          if (is.null(p)) ggplot() + theme_void() + coord_fixed()
          else p + coord_fixed()
        }
