...
# v.3.0-3.5 Bootstrapping ------------------------------------------------------
# Bootstrapping",

# Doesnt impact calobration - just produces 
...
# ===============================================================
  ...
  # ===============================================================
  # 4. THEME & MODEL LABELS & BOOTSTRAP SET UP
  # ===============================================================
  theme_small <- theme(
  ...
  # ===============================================================
  # 8.0 Set up - seeds, progress bars, global stores, bootstrap performance --------
  ...
  # 8.1-8.5 Fit models, Evaluate models, Boostrapping, Plotting, Save outputs ------------------
    ...
    # ELEMENT LOOP     
      ...
      # ---------------------------------------------------------------
      # MODELS (never skip; failures -> NULL)
      # ---------------------------------------------------------------
        model_list <- list(
  ...
  # 8.3 Bootstrap diagnostics -----------------------------------------------
        ...
        if (do_predictions) {
          ...
          # log-space % error AND ppm-scale % error intervals
  ...
  # 8.4 Plotting ------------------------------------------------------------
        ...
        # Number of datapoints actually plotted
        n_points <- sum(!is.na(y) & !is.na(pred))
        #"\nR² = ", signif(R2, 3),
        
        # ---- Bootstrap RMSE_ppm SD lookup (SAFE) ----
        RMSE_ppm_sd <- NA_real_
        ...
        if (length(bootstrap_store) > 0) {
          ...
          if (nrow(boot_row) == 1 && "RMSE_ppm_sd" %in% names(boot_row)) {
        ...
        stats_label <- paste0(
          ...
          "\nRMSEP = ",
          signif(RMSE_ppm, 4),
          if (!is.na(RMSE_ppm_sd))
            paste0(" \u00B1 ", signif(RMSE_ppm_sd, 3))
          else ""
        )
      ...
      # PLOT 1. Pred vs Obs plots WITH error bars (per element & model)
        ...
        ann_x <- axis_min_pad + 0.01 * (axis_max_pad - axis_min_pad)
        ann_y <- axis_max_pad - 0.01 * (axis_max_pad - axis_min_pad)
  ...
  # 8.5 Save outputs --------------------------------------------------------
    ...
    # MODEL RANKING & DIAGNOSTIC OUTPUT (PATCHED)
  ...
  # ===============================================================  
    # SECTION 11.0: GLOBAL SUMMARY OF ALL ELEMENTS AND ALL MODELS (PATCHED)
    # ===============================================================
      
      message("=== Building global calibration summary (patched Section 11.0) ===")
      
      # ---- helpers (used throughout 11.0–11.5) ----
      ...
      global_summary <- ensure_cols(
        global_summary,
        c("Mean_ppm","Mean_CI_width","SNR_model","SNR_CI","SNR_smooth","Roughness"),
        fill = NA_real_
      )
      ...
      if (nrow(snr_df_global) > 0) {
        global_summary <- global_summary %>%
          dplyr::left_join(
            snr_df_global %>%
              safe_select(c("Element","Model","Mean_ppm","Mean_CI_width","SNR_model","SNR_CI","SNR_smooth","Roughness")),
            by = c("Element","Model"),
            suffix = c("", "_snr")
          ) %>%
          dplyr::mutate(
      ...
      # ---- Production model selection (unchanged logic, made safe) ----
      production_models <- global_summary %>%
        ...
        dplyr::slice(1) %>%
        dplyr::ungroup() %>%
        dplyr::mutate(Production_Model = Model)
      ...
      global_summary <- global_summary %>%
        dplyr::mutate(
          ...
          Confidence_Flag = dplyr::case_when(
            Robustness_score >= 0.75 & !Unstable_flag ~ "High confidence (signal-dominated)",
            SNR_CI_s < 0.4 & Robustness_score < 0.6   ~ "Low confidence (uncertainty-dominated)",
            Unstable_flag == TRUE                      ~ "Low confidence (noise-dominated)",
            Robustness_score >= 0.6 & Robustness_score < 0.75 ~ "Moderate confidence",
            TRUE ~ "Low confidence"
          ),
          ...
          Confidence_class = dplyr::case_when(
            Robustness_score >= 0.75 & !Bootstrap_Unstable & !Unstable_flag ~ "High confidence",
      ...
      # ---- 11.1E Audit tables (ALL + per-site list placeholders) ----
      audit_all <- global_summary %>%
        safe_select(c(
          "Element","Model",
          "R2","RMSE","RMSE_ppm","RMSEP",
          "Mean_ppm","Mean_CI_width",
          "SNR_model","SNR_CI","SNR_smooth","Roughness",
          "SNR_CI_s","SNR_smooth_s","Roughness_norm",
          "Robustness_score","Robust_Rank","Global_Rank",
          "Robustness_Class","Stability_class","Confidence_class","Confidence_Flag",
      ...
      write.csv(
        audit_all,
        file.path(robust_root, "AllElementsModel_Robustness_Stability_Conf_Boot_ALL.csv"),
        row.names = FALSE
      )
    ...
    # ===============================================================
      ...
      # 2. STABILITY CLASS HEATMAP (GLOBAL)
      ...
      p_heat_stability <- ggplot(
        ...
        aes(
          ...
          fill = dplyr::case_when(
            ...
            Robustness_Class == "Marginal"   ~ 0.33,
            Robustness_Class == "Acceptable"~ 0.67,
            Robustness_Class == "Stable"     ~ 1,
            TRUE ~ NA_real_
          )
        )
      ) +
      ...
      # 3. CONFIDENCE CLASS HEATMAP (GLOBAL)
      ...
      p_heat_confidence <- ggplot(
        ...
        aes(
          ...
          y = Element,
          fill = dplyr::case_when(
            Confidence_class == "Signal-dominated"      ~ 1,
            Confidence_class == "Balanced"              ~ 0.5,
            Confidence_class == "Uncertainty-dominated" ~ 0,
            TRUE ~ NA_real_
          )
        )
      ) +
    ...
    # ===============================================================
      ...
      global_bands <- global_summary %>%
        safe_select(c("Element","Model","Robustness_low","Robustness_high","Bootstrap_Unstable","Global_Rank","Robustness_score"))
      ...
      write.csv(snr_df, file.path(robust_root, "Robustness_ALL_SITES.csv"), row.names = FALSE)
      for (s in unique(snr_df$Site)) {
        write.csv(dplyr::filter(snr_df, Site == s),
                  file.path(robust_root, paste0("Robustness_Site_", s, ".csv")),
                  row.names = FALSE)
      }
      message("✓ Robustness CSVs written (11.3)")
      
      # Build per-site audit tables now that snr_df exists
      global_metrics <- global_summary %>%
        safe_select(c("Element","Model","R2","RMSE","RMSE_ppm","RMSEP","Global_Rank","Robustness_score","Robustness_Class","Confidence_Flag","Bootstrap_Unstable"))
      ...
      for (s in sites_out) {
        out <- dplyr::filter(site_audit, Site == s) %>% dplyr::arrange(Element, Rank_site)
        write.csv(out,
    ...
    # (kept, simplified; outputs unchanged)
    # ===============================================================
      ...
      write.csv(
        site_audit %>%
          safe_select(c(
            ...
            "Robustness_score_site","R2_overall","RMSE_overall","RMSEP_overall",
    ...
    # ===============================================================
    # SECTION 11.5: SINGLE MASTER AUDIT CSV (ALL + PER-SITE)
    # ===============================================================
      
      audit_all_master <- audit_all %>%
        dplyr::mutate(Site = "ALL") %>%
        dplyr::select(Site, dplyr::everything())
      
      audit_site_master <- site_audit %>%
        dplyr::mutate(Site = as.character(Site)) %>%
        dplyr::select(
          Site, Element, Model,
          Rank_site,
          Robustness_score_site,
          SNR_CI_s, SNR_smooth_s,
          n_mean,
          R2_overall, RMSE_overall, RMSEP_overall,
          Global_Rank,
          Robustness_score,
          Robustness_Class,
          Confidence_Flag,
      ...
      audit_master <- dplyr::bind_rows(audit_all_master, audit_site_master) %>%
        dplyr::arrange(Site, Element, dplyr::coalesce(Rank_site, Global_Rank))
      
      write.csv(
        audit_master,
        file.path(robust_root, "MASTER_Audit_ALL_and_PerSite.csv"),
        row.names = FALSE
      )
      
      message("✓ MASTER audit written: MASTER_Audit_ALL_and_PerSite.csv")
