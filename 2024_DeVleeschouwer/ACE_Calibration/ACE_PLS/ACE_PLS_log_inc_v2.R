# -------------------------------------------------------------------------
# PLS validation tests and final 4-element calibration model
# XRF-CS log_inc scaled/centered predictor model and ICP response dataset
# -------------------------------------------------------------------------

# -------------------------------------------------------------------------
# ACE datasets
# -------------------------------------------------------------------------
# Subsample dataset of measured ICP-MS depth-matched inc. normalised XRF-CS data n=268     
# XRF core scanning n=14513 for prediction (XRF_new)     
# Figure 3 ACE PLS log_inc using pls package
# Figure 4 ACE PSL as ppm plots
# -------------------------------------------------------------------------
...
# Select dataset to use as df0, convert to base R dataframe for PLS package --------
df0 <- ACE_matched_xrf_icp_log_inc %>%
  dplyr::select(c(Site, depth, SH20_mean_age, SH20_mean_95CI, all_of(xrf_icp_elements1))) %>%
  drop_na()
df0
...
#-------------------------------------------------------------------------------
# 1. Partial Least Squares (PLS) optimal no. of components & validation
# ------------------------------------------------------------------------------
# Define X_test & Y_test for Ti test to use --------------------------------------------------
# Validation models - single element test - XRF as predictors (X) and ICP-MS response (Y)
...
# PLSR-LOO (leave-one-out) Ti test model to determine optimal number of comps -----
...
pls_train_model_LOO <- plsr(Y_test ~ ., data = data.frame(X_test, Y_test), 
                            validation = "LOO", jackknife = TRUE, scale = TRUE, center = TRUE) # Use scale = TRUE to perform as Z scores - do not use for calibration
...
capture.output(summary(pls_train_model_LOO), file = "Papers_R/2024_DeVleeschouwer/ACE_Calibration/ACE_PLS/Output/PLSR/log_inc/1_PLSR_ncomp_outputs/PLS_test_Ti/PLS_model_LOO_summary_Ti.txt")
...
pdf("Papers_R/2024_DeVleeschouwer/ACE_Calibration/ACE_PLS/Output/PLSR/log_inc/1_PLSR_ncomp_outputs/PLS_test_Ti/PLS_rmsep_plot_LOO.pdf", width = 4, height = 4)
...
# PLSR k-fold CV (cross-validated) Ti test model to determine optimal number of comps ----

# Calibrate Titanium (Ti) measured by ICP-MS against other elements measured by ITRAX using pls package

# Perform PLS regression with 10-fold cross-validation for model tuning - useful for small datasets
pls_train_model_CV <- plsr(Y_test ~ ., data = data.frame(X_test, Y_test), 
                           validation = "CV", jackknife = TRUE, scale = TRUE, center = TRUE) # Use scale = TRUE to perform as Z scores - do not use for calibration
...
# Open a PDF in device
pdf("Papers_R/2024_DeVleeschouwer/ACE_Calibration/ACE_PLS/Output/PLSR/log_inc/1_PLSR_ncomp_outputs/PLS_test_Ti/PLS_rmsep_plot_CV.pdf", width = 4, height = 4)
...
# -------------------------------------------------------------------------
# 1.1 Function to determine optimal number of comps all elements LOO & k-fold CV
# -------------------------------------------------------------------------
# Import XRF_new for prediction of values -------------------------------
...
XRF_new <- readr::read_csv(
  ...
  dplyr::mutate(across(all_of(key_elements_xrf), ~ ifelse(. <= -10, NA, .))) %>% 
  print()
str(XRF_new)

# PLSR CV Function --------------------------------------------------------------
run_plsr_cv_all_elements <- function(
    df,
    key_elements_xrf,
    key_elements_icp,
    output_dir,
    newdata = NULL,                  # optional new XRF dataset for prediction
    cv_k = 10,                       # k for k-fold CV
    segments_type = c("random", "consecutive", "interleaved"),
    ncomp_max = NULL,                # optional cap; default derived from data
    scale = TRUE,
    center = TRUE,
    save_per_element_rmsep_pdf = TRUE,
    save_combined_rmsep_pdf = TRUE
) {
  
  # Packages
  require(pls)
  
  segments_type <- match.arg(segments_type)
  
  # Checks
  stopifnot(length(key_elements_xrf) == length(key_elements_icp))
  stopifnot(all(key_elements_xrf %in% names(df)))
  stopifnot(all(key_elements_icp %in% names(df)))
  
  if (!is.null(newdata)) {
    stopifnot(all(key_elements_xrf %in% names(newdata)))
  }
  
  dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)
  
  # Helper: choose ncomp_max sensibly
  choose_ncomp_max <- function(n, p) {
    # Upper bound heuristic to avoid overfitting / degenerate comps
    max(1L, min(p, n - 2L))
  }
  
  # Storage
  results_summary <- data.frame(
    Element_ICP = character(),
    Element_XRF = character(),
    n_obs = integer(),
    n_pred = integer(),
    ncomp_max = integer(),
    ncomp_opt_LOO = integer(),
    rmsep_min_LOO = numeric(),
    ncomp_opt_kfold = integer(),
    rmsep_min_kfold = numeric(),
    stringsAsFactors = FALSE
  )
  
  preds_list <- list()   # per-element predictions (if newdata provided)
  rmsep_store <- list()  # RMSEP objects per element (LOO + kfold)
  
  # Use same CV splits across elements when random segments are used
  # (keeps comparability). Works because rows used are those non-NA in Y.
  set.seed(1)
  
  # ------------------------------------------------------------
  # Loop elements
  # ------------------------------------------------------------
  for (i in seq_along(key_elements_icp)) {
    
    icp_el <- key_elements_icp[i]
    xrf_el <- key_elements_xrf[i]
    
    message("Running PLS CV for: ", icp_el)
    
    # Build modelling frame with complete cases for this Y
    X_all <- df[, key_elements_xrf, drop = FALSE]
    y_all <- df[[icp_el]]
    ...
    if (n < 5) {
      warning("Skipping ", icp_el, " (too few complete rows after NA removal).")
      next
    }
    
    ncomp_max_i <- if (is.null(ncomp_max)) choose_ncomp_max(n, p) else min(ncomp_max, choose_ncomp_max(n, p))
    
    pls_df <- data.frame(X, Y_test = y)
    ...
    rmsep_LOO <- RMSEP(pls_LOO)
    # rmsep_LOO$val dims: [estimate, response, ncomp+1] with first = 0 comps
    loo_vals <- rmsep_LOO$val[1, 1, -1]
    ncomp_opt_LOO <- which.min(loo_vals)
    rmsep_min_LOO <- min(loo_vals)
    
    # -----------------------
    # k-fold model
    # -----------------------
    # pls uses "CV" with 'segments' for k-fold
    # segments = cv_k gives k folds; segment.type controls split behaviour
    pls_kfold <- plsr(
      Y_test ~ .,
      data = pls_df,
      validation = "CV",
      segments = cv_k,
      segment.type = segments_type,
      jackknife = TRUE,
      scale = scale,
      center = center,
      ncomp = ncomp_max_i
    ...
    rmsep_kfold <- RMSEP(pls_kfold)
    k_vals <- rmsep_kfold$val[1, 1, -1]
    ncomp_opt_kfold <- which.min(k_vals)
    rmsep_min_kfold <- min(k_vals)
    
    # Store RMSEP objects for combined plotting
    rmsep_store[[xrf_el]] <- list(LOO = rmsep_LOO, kfold = rmsep_kfold)
    
    # -----------------------
    # Save per-element text summary (includes both CV types)
    # -----------------------
    summary_file <- file.path(output_dir, paste0("PLS_model_CV_summary_", xrf_el, ".txt"))
    capture.output(
      {
        cat("ICP element:", icp_el, "\n")
        cat("XRF elements used:", paste(key_elements_xrf, collapse = ", "), "\n")
        cat("Rows used (complete cases):", n, "\n")
        cat("ncomp_max:", ncomp_max_i, "\n\n")
        
        cat("=== LOO CV ===\n")
        cat("Optimal ncomp (LOO):", ncomp_opt_LOO, "\n")
        cat("Min RMSEP (LOO):", rmsep_min_LOO, "\n\n")
        print(summary(pls_LOO))
        cat("\n--- Jackknife test (LOO optimum) ---\n")
        print(jack.test(pls_LOO, ncomp = ncomp_opt_LOO, use.mean = TRUE))
        
        cat("\n\n=== k-fold CV ===\n")
        cat("k:", cv_k, " segment.type:", segments_type, "\n")
        cat("Optimal ncomp (k-fold):", ncomp_opt_kfold, "\n")
        cat("Min RMSEP (k-fold):", rmsep_min_kfold, "\n\n")
        print(summary(pls_kfold))
        cat("\n--- Jackknife test (k-fold optimum) ---\n")
        print(jack.test(pls_kfold, ncomp = ncomp_opt_kfold, use.mean = TRUE))
      },
      file = summary_file
    )
    
    # -----------------------
    # Save per-element RMSEP PDFs (optional)
    # -----------------------
    if (save_per_element_rmsep_pdf) {
      pdf(file.path(output_dir, paste0("PLS_RMSEP_LOO_", xrf_el, ".pdf")), width = 4, height = 4)
      plot(rmsep_LOO, legendpos = "topright", main = paste0(xrf_el, " (LOO)"))
      dev.off()
      
      pdf(file.path(output_dir, paste0("PLS_RMSEP_kfold_", xrf_el, ".pdf")), width = 4, height = 4)
      plot(rmsep_kfold, legendpos = "topright", main = paste0(xrf_el, " (k=", cv_k, ")"))
      dev.off()
    }
    
    # -----------------------
    # Auto prediction pipeline (optional):
    # Fit "final" model using chosen optimum (default: k-fold optimum)
    # Then predict for newdata.
    # -----------------------
    n_pred <- 0L
    if (!is.null(newdata)) {
      
      X_new <- newdata[, key_elements_xrf, drop = FALSE]
      # allow NAs in newdata: predictions will be NA where predictors missing
      can_pred <- complete.cases(X_new)
      n_pred <- sum(can_pred)
      
      # Fit final model using k-fold optimum ncomp
      final_model <- plsr(
        Y_test ~ .,
        data = pls_df,
        validation = "none",
        scale = scale,
        center = center,
        ncomp = ncomp_opt_kfold
      )
      
      pred_vec <- rep(NA_real_, nrow(X_new))
      pred_vec[can_pred] <- as.numeric(predict(final_model, newdata = X_new[can_pred, , drop = FALSE], ncomp = ncomp_opt_kfold))
      
      preds_list[[icp_el]] <- data.frame(
        row_id = seq_len(nrow(X_new)),
        Element_ICP = icp_el,
        Element_XRF = xrf_el,
        ncomp_used = ncomp_opt_kfold,
        Pred = pred_vec
      )
      
      # Save per-element predictions CSV
      write.csv(
        preds_list[[icp_el]],
        file = file.path(output_dir, paste0("PLS_predictions_kfoldOpt_", xrf_el, ".csv")),
        row.names = FALSE
      )
    }
    ...
    results_summary <- rbind(
      ...
      data.frame(
        ...
        n_obs = n,
        n_pred = n_pred,
        ncomp_max = ncomp_max_i,
        ncomp_opt_LOO = ncomp_opt_LOO,
        rmsep_min_LOO = rmsep_min_LOO,
        ncomp_opt_kfold = ncomp_opt_kfold,
        rmsep_min_kfold = rmsep_min_kfold,
        stringsAsFactors = FALSE
      )
    )
  }
  ...
  # ------------------------------------------------------------
  # Combined RMSEP multi-panel PDF (optional)
  # Two pages: LOO and k-fold
  # ------------------------------------------------------------
  if (save_combined_rmsep_pdf && length(rmsep_store) > 0) {
    
    els <- names(rmsep_store)
    m <- length(els)
    
    # Simple grid: try to make it ~square
    ncol <- ceiling(sqrt(m))
    nrow <- ceiling(m / ncol)
    
    pdf(file.path(output_dir, "PLS_RMSEP_ALL_ELEMENTS_multiPanel.pdf"),
        width = 4 * ncol, height = 4 * nrow)
    
    # Page 1: LOO
    par(mfrow = c(nrow, ncol), mar = c(4, 4, 3, 1))
    for (el in els) {
      plot(rmsep_store[[el]]$LOO, legendpos = "topright", main = paste0(el, " (LOO)"))
    }
    # Fill any empty slots
    if (m < nrow * ncol) for (k in seq_len(nrow * ncol - m)) plot.new()
    
    # Page 2: k-fold
    par(mfrow = c(nrow, ncol), mar = c(4, 4, 3, 1))
    for (el in els) {
      plot(rmsep_store[[el]]$kfold, legendpos = "topright",
           main = paste0(el, " (k=", cv_k, ")"))
    }
    if (m < nrow * ncol) for (k in seq_len(nrow * ncol - m)) plot.new()
  ...
  write.csv(results_summary,
            file = file.path(output_dir, "PLS_CV_summary_ALL_ELEMENTS.csv"),
            row.names = FALSE)
  
  # Bind predictions to one data frame (if any)
  preds_df <- if (length(preds_list) > 0) do.call(rbind, preds_list) else NULL
...
out <- run_plsr_cv_all_elements(
  ...
  cv_k = 10,
  segments_type = "random"
)

# Combined predictions table (all elements stacked)
...
#-------------------------------------------------------------------------------
# 2. RMSE & Jackknifing tests validation (make into a function)                
# ------------------------------------------------------------------------------
# PLS-LOO - RMSEP & Jackknifing - all - write console to txt file -------------------
...
# PLS k-fold CV - RMSEP & Jackknifing - all - write console to txt file -------------------
...
# ------------------------------------------------------------------------------
# 3. PSLR training model with Ti, Ca, Sr, Zr as element matrix                 
# ------------------------------------------------------------------------------
# Select variables to use  ------------------------------------------------

# Only use signifcant predictor ITRAX variables from LOO & CV jackknife results for final model

# final_elements_xrf <- c("Ti", "Ca", "Fe", "Mn", "Sr", "Zr")
# final_elements_icp <- c("Ti_ICP", "Ca_ICP", "Fe_ICP", "Mn_ICP", "Sr_ICP", "Zr_ICP")
# opt_comp_final = 6
# 
...
# Plot predicted vs measured draft output
...
# R2_corr, RMSE predictions for final model base R ----------------------------------
...
# Generate R2, RMSE, equation text - not needed anymore - now automated ------------------------
# # Ti
...
# Open a PDF in device
pdf("Papers_R/2024_DeVleeschouwer/ACE_Calibration/ACE_PLS/Output/PLSR/log_inc/PLS_BIC_plot_4elements.pdf", width = 4, height = 4)
...
# Rename based on measured column names
...
# Plot Ti pred vs measured as log data, coloured by Site in ggplot ---------------
...
# Plot Ca pred vs measured as log data, coloured by Site in ggplot ----------------
...
# Plot Sr pred vs measured as log data, coloured by Site in ggplot ----------------
...
# Plot Zr pred vs measured as log data, coloured by Site in ggplot ----------------
...
# Convert to ppm & plot as measured vs measured in ppm  --------------------------
...
# Plot Ti pred vs measured as ppm, coloured by Site in ggplot -----------------------
...
# Plot Ca pred vs measured as ppm, coloured by Site in ggplot -----------------------
...
# Plot Sr pred vs measured as ppm, coloured by Site in ggplot -----------------------
...
# Plot Zr pred vs measured as ppm, coloured by Site in ggplot -----------------------
...
#-------------------------------------------------------------------------------
# 6. PLS Train-Test Split tidyverse method  ------------------------------------
...
# 6.1 Ti training:test & bootstrapping -----------------------------------
...
# 6.2 Predict Ti as ppm using pls_final model and Ln(Ti/inc) ITRAX dataset ---------
...
# 6.3 PLS Train-Test Split Base R & ggplot method [OPTIONAL] --------------------
...
# Plot predicted vs measured on test dataset
...
# Plot predictions vs measured, colored by Site in ggplot
