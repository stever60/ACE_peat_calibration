# Figure 2 and Figure S3 - Correlation - log_inc

# 8/9/25: depth matching now defined by Subsample-ICPMS matched composite dataset
# n = 268 - increased from n = 265 after reduction from n = 272 - corrections to composite depths in BI10 10/6/25 and then KER3 8/9/25s
# Code adapted from "Papers_R/2024_DeVleeschouwer/Output/itrax_Composite/Matching_mean/ACE/ACE_matching_log_inc.R" 
...
# # Run ONCE - Import ACE cps file & calculate xrf fractional errors ------------------------
...
# 
# # Run ONCE - Create log_inc normalised xrf dataset and log transformed ICPMS dataset -------
# ACE_subsample_xrf_icp_matched_log_inc <- ACE_subsample_xrf_icp_matched_cps_fr_err %>% 
#   filter(!if_any(everything(), is.na)) %>% # remove rows will all NAs
# # XRF data - normalise XRf elements by inc. scatter
#   mutate(across(all_of(c(acf_icp_Elements_min)), .fns = ~./ Mo_inc)) %>% # 
#   mutate_at(vars(all_of(c(acf_icp_Elements_min))), ~ (. == 0) * min(.[. != 0])/2 + .) %>% # Replace zeros with half minimum value to allow logs
# # ICPMS data - replace any ICPMS values <0 with zero
#   mutate(across(all_of(c(icp_Elements_min)), ~ ifelse(.x < 0, 0, .x))) %>%  # 
#   mutate_at(vars(all_of(c(icp_Elements_min))), ~ (. == 0) * min(.[. != 0])/2 + .) %>% ## Replace zeros with half minimum value to allow logs
# # log XRF and ICPMS data  
#   mutate(across(all_of(c(acf_icp_Elements_min, icp_Elements_min)), log)) %>% #log all xrf and icp data
...
#   mutate(across(all_of(acf_icp_Elements_min_sd), abs)) %>% # change negative SD error values to absolute (positive) values
# # calculate percentage errors for ICPMS from analytical measurement errors
...
#   print()
# is.na(ACE_subsample_xrf_icp_matched_log_inc)<-sapply(ACE_subsample_xrf_icp_matched_log_inc, is.infinite) # replace any infinite values with NA
# ACE_subsample_xrf_icp_matched_log_inc
# write.csv(ACE_subsample_xrf_icp_matched_log_inc,"Papers_R/2024_DeVleeschouwer/Figure2/Data/Output/ACE_subsample_xrf_icp_matched_log_inc.csv", row.names = FALSE)
# write.csv(ACE_subsample_xrf_icp_matched_log_inc,"Papers_R/2024_DeVleeschouwer/FigureS6/Data/Input/ACE_subsample_xrf_icp_matched_log_inc.csv", row.names = FALSE)
# write.csv(ACE_subsample_xrf_icp_matched_log_inc,"Papers_R/2024_DeVleeschouwer/Figure4/Data/Input/ACE_subsample_xrf_icp_matched_log_inc.csv", row.names = FALSE)
# 
# 
# 
...
ACE_subsample_xrf_icp_matched_log_inc <-read_csv("Papers_R/2024_DeVleeschouwer/Figure2/Data/Output/ACE_subsample_xrf_icp_matched_log_inc.csv")

# Define dataset to use for Linear modelling  ----------------------------------
ACE_LM1 <- ACE_subsample_xrf_icp_matched_log_inc
...
ACE_LM1_stats <- ACE_LM1 %>%
  ...
  print()
write.csv(ACE_LM1_stats,"Papers_R/2024_DeVleeschouwer/Figure2/Data/Output/ACE_subsample_xrf_icp_matched_stats_log_inc_.csv", row.names = FALSE)
...
ggpairs(ACE_LM1, columns = xrf_icp_Elements_key_Fig2, upper = list(continuous = wrap("cor", size = 2)),
        ...
        title="ACE XRF-CS & ICPMS: Correlation-density plot - log_inc")
ggsave("Papers_R/2024_DeVleeschouwer/Figure2/Plots/FigS3.1_itrax_ICP_corr-den_matrix_log_inc.pdf", 
...
ggsave("Papers_R/2024_DeVleeschouwer/Figure2/Plots/FigS3.2_itrax_ICP_corr-den_sites_log_inc.pdf", 
       height = c(30), width = c(30), dpi = 600, units = "cm")
...
ggsave("Papers_R/2024_DeVleeschouwer/Figure2/Plots/FigS3.3_itrax_ICP_corr-den_sites_all_log_inc.pdf", 
       height = c(30), width = c(30), dpi = 600, units = "cm")
