...
# sink(file = "Papers_R/2024_DeVleeschouwer/ACE_PLS/Output/PLSR/Ti/log_inc/Summary_stats.txt")
...
df_predicted
write.csv(df_predicted,"Papers_R/2024_DeVleeschouwer/ACE_PLS/Output/PLSR/Ti/log_inc/Ti_ICP_predicted.csv", row.names = FALSE)
...
PLS_final <- ggplot(df_predicted, aes(x = Ti_ICP, y = Ti_predicted, color = Site)) +
  ...
  labs(
    title = "PLS Regression: Measured vs Predicted log_inc model",
  ...
  coord_equal()
print(PLS_final)
ggsave("Papers_R/2024_DeVleeschouwer/ACE_PLS/Output/PLSR/Ti/log_inc/Ti_plsr_final.pdf", 
...
y_pred_scaled_exp <- exp(y_pred_scaled)
y_pred_scaled_RMSE_upper_exp <- exp(y_pred_scaled_RMSE_upper)
y_pred_scaled_RMSE_lower_exp <- exp(y_pred_scaled_RMSE_lower)
...
Ti_ppm_PLS1 <- tibble(Ti_pred_log_inc = y_pred_scaled,
                      Ti_ppm_PLS_log_inc = y_pred_scaled_exp, 
                      Ti_upper_PLS_log_inc = y_pred_scaled_RMSE_upper_exp,
                      Ti_lower_PLS_log_inc = y_pred_scaled_RMSE_lower_exp)
Ti_ppm_PLS_predicted <- bind_cols(ACE_xrf_predict_Ti_PLS, Ti_ppm_PLS1)
write.csv(Ti_ppm_PLS_predicted,"Papers_R/2024_DeVleeschouwer/ACE_PLS/Output/PLSR/Ti/log_inc/ACE_Ti_ppm_PLS_predicted.csv", row.names = FALSE)

# Extract HER42PB data for Figure 4
HER42PB_Ti_ppm_PLS_predicted <- Ti_ppm_PLS_predicted %>% 
  filter(Site == "HER42PB")
HER42PB_Ti_ppm_PLS_predicted
write.csv(HER42PB_Ti_ppm_PLS_predicted,"Papers_R/2024_DeVleeschouwer/ACE_PLS/Output/PLSR/Ti/log_inc/HER42PB_Ti_ppm_PLS_predicted.csv", row.names = FALSE)
...
df_Site <- df[, (names(df) %in% c("Site", "Ti_ICP", final_elements_xrf))] # or use 6 key_elements_xrf 
...
train_df <- df_split %>%
  group_by(Site) %>%
  slice_sample(prop = 0.6) %>% # set to 0.6: 0.4 split as this matches R2 = 0.75 and rmse from whole dataset final model
  ungroup()
...
for (i in 1:n_boot) {
  ...
  # 4. Calculate RMSE in original units
  actual <- test_df$Ti_ICP
  rmse_vals[i] <- sqrt(mean((actual - pred)^2))
}

## Analyze the RMSE distribution
...
# Optional: Plot the RMSE distribution
hist(rmse_vals, breaks = 30, col = "skyblue", main = "Bootstrap RMSE Distribution", xlab = "RMSE")
...
# Predict Ti using pls_final model and bootstrap mean rmse value - for ppm reconstruction
y_pred_scaled_bs <- predict(pls_final, newdata = X_new, ncomp = 3)
y_pred_scaled_RMSE_upper_bs <- y_pred_scaled + mean_rmse # mean RMSE from Bstrap
y_pred_scaled_RMSE_lower_bs <- y_pred_scaled - mean_rmse # mean RMSE from Bstrap
# Y was log-transformed - therefore take exp of both sides to convert to ppm 
...
write.csv(ACE_xrf_conversion_Ti_PLS,"Papers_R/2024_DeVleeschouwer/ACE_PLS/Output/PLSR/Ti/log_inc/ACE_plsr_xrf_Ti_converted_PLS.csv", row.names = FALSE)
